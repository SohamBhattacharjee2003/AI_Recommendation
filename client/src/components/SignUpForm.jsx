...
const SignUpForm = ({ buttonClasses, buttonForGFT }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      // Navigate to the first step of the apply wizard
  ...
  const handleGoogleSignUp = async () => {
    try {
      ...
      if (result.user) {
        if (!result.user.displayName && formData.fullName) {
        ...
        // Navigate to the first step of the apply wizard
        navigate("/apply/about", { replace: true });
      }
    } catch (err) {
      ...
      if (err.code === 'auth/account-exists-with-different-credential') {
        ...
        if (email) {
          ...
          setError(
            ...
            `Please sign in with ${methods.join(', ')}.`
          );
        } else {
    ...
    <div className="w-full bg-white rounded-lg shadow-xl md:mt-0 sm:max-w-md xl:p-0 border border-gray-100">
      <div className="p-6 space-y-6 md:space-y-7 sm:p-8">
        ...
        <div className="grid grid-cols-3 gap-3">
          {/* Google */}
          <button onClick={handleGoogleSignUp} className={buttonForGFT}>
            <svg
              className="h-5 w-5"
              aria-hidden="true"
              fill="currentColor"
          ...
          {/* GitHub */}
          <button onClick={handleGithubSignUp} className={buttonForGFT}>
            <svg
              className="h-5 w-5"
              aria-hidden="true"
              fill="currentColor"
