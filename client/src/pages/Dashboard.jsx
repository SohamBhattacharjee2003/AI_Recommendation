...
function Dashboard() {
  ...
  const [skillsAssessment] = useState(70);
  const [careerGoals] = useState(90);
  ...
  useEffect(() => {
    ...
    const handleScroll = () => {
      ...
      const scrollingUp = currentPosition < lastScrollPos;
      setLastScrollPos(currentPosition);

      // Hide scroll indicator after scrolling down
      setShowScrollIndicator(currentPosition <= 100);

      const sections = [
        { ref: progressSectionRef, setVisible: setProgressVisible },
        { ref: featuresSectionRef, setVisible: setFeaturesVisible },
        { ref: calendarSectionRef, setVisible: setCalendarVisible },
        { ref: suggestionsSectionRef, setVisible: setSuggestionsVisible },
        { ref: quickActionsSectionRef, setVisible: setQuickActionsVisible }
      ];
            ...
            {upcomingEvents.map((event) => (
              <div
                ...
                className={`
                  ...
                  shadow-sm border-l-4 border-blue-400 hover:shadow-md 
        ...
        >
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
            {recommendations.map((rec) => (
              <div
                key={rec.id}
                className="bg-gradient-to-br from-gray-50 to-white p-4 rounded-lg shadow-sm hover:shadow-md transition-all duration-300 group flex flex-col h-full" // Added flex flex-col and h-full
              >
                ...
                <div className="flex-grow"> {/* Added flex-grow to push button to bottom */}
                  <h3 className="font-medium text-gray-800 mb-2">{rec.title}</h3>
                  <p className="text-sm text-gray-600">{rec.reason}</p>
                </div>
                <button className="w-full mt-4 flex items-center justify-between px-3 py-2 bg-white border border-gray-200 rounded-md hover:border-blue-400 transition-all duration-300">
